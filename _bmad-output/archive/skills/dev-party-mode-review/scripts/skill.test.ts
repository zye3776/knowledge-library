...
function validateInput(input: Partial<SkillInput>): ValidationResult {
  ...
  if (!input.story_context) {
    ...
    if (!input.story_context.name) errors.push("story_context.name is required");
  ...
  if (input.options) {
    if (input.options.discussion_depth &&
        !["brief", "standard", "thorough"].includes(input.options.discussion_depth)) {
      errors.push("options.discussion_depth must be 'brief', 'standard', or 'thorough'");
    }
    if (input.options.max_discussion_rounds !== undefined &&
        (input.options.max_discussion_rounds < 1 || input.options.max_discussion_rounds > 10)) {
...
function validateOutput(output: SkillOutput): ValidationResult {
  ...
  if (output.success) {
    ...
    if (!successOutput.result) {
      ...
      if (!["approved", "approved_with_reservations", "needs_revision"].includes(successOutput.result.status)) {
        errors.push("result.status must be 'approved', 'approved_with_reservations', or 'needs_revision'");
      }
      if (typeof successOutput.result.confidence_score !== "number" ||
          successOutput.result.confidence_score < 0 ||
          successOutput.result.confidence_score > 100) {
        errors.push("result.confidence_score must be a number between 0 and 100");
      }
      ...
      if (!successOutput.result.agent_consensus) {
        ...
        const validVotes: AgentVote[] = ["approve", "approve_with_notes", "reject"];
        for (const agent of ["architect", "developer", "tech_lead", "qa_engineer"]) {
          const vote = successOutput.result.agent_consensus[agent as keyof typeof successOutput.result.agent_consensus];
          if (typeof vote === "string" && !validVotes.includes(vote as AgentVote)) {
            errors.push(`result.agent_consensus.${agent} must be 'approve', 'approve_with_notes', or 'reject'`);
          }
        }
      }
      if (!successOutput.result.quality_assessment) {
        ...
        for (const metric of ["implementability", "completeness", "clarity", "architectural_fit"]) {
          const value = successOutput.result.quality_assessment[metric as keyof typeof successOutput.result.quality_assessment];
          if (typeof value !== "number" || value < 0 || value > 100) {
            errors.push(`result.quality_assessment.${metric} must be a number between 0 and 100`);
          }
        }
      }
    }
...
function determineConsensusStatus(consensus: SkillOutputSuccess["result"]["agent_consensus"]): ReviewStatus {
  const votes = [
    consensus.architect,
    consensus.developer,
    consensus.tech_lead,
...
describe("Consensus Logic", () => {
  ...
  it("should return 'approved_with_reservations' when some approve with notes", () => {
    ...
    expect(determineConsensusStatus(consensus)).toBe("approved_with_reservations");
  });
...
describe("Review Scenarios", () => {
  describe("Scenario: Clean plan approval", () => {
    it("should produce approved status with all issues fixed", () => {
      ...
      expect(output.result.changes_made.every((c) => c.change_type === "fixed")).toBe(true);
    });
  });
  ...
  describe("Scenario: Plan with critical architecture violation", () => {
    it("should result in needs_revision when architect rejects", () => {
      ...
      output.result.status = determineConsensusStatus(output.result.agent_consensus);
  ...
  describe("Scenario: Agent disagreement", () => {
    it("should capture dissenting opinions", () => {
      ...
      expect(output.result.agent_consensus.dissenting_opinions.length).toBeGreaterThan(0);
      expect(output.result.agent_consensus.dissenting_opinions[0].concern).toBeTruthy();
    });
  });
});
...
describe("Change Documentation", () => {
  it("should link changes to finding IDs", () => {
    ...
    for (const change of output.result.changes_made) {
      ...
      expect(change.change_type).toMatch(/^(fixed|partially_addressed|deferred)$/);
  ...
  it("should have valid change_type values", () => {
    const validTypes: ChangeType[] = ["fixed", "partially_addressed", "deferred"];
    const output = createMockSuccessOutput();
...
describe("Metadata Validation", () => {
  ...
  it("should list all participating agents", () => {
    const output = createMockSuccessOutput();
    const expectedAgents = ["architect", "developer", "tech_lead", "qa_engineer"];
