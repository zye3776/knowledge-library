...
program
  ...
  .option("-r, --rate <rate>", `Playback rate ${MIN_RATE}-${MAX_RATE} (default: ${DEFAULT_SPEED})`, String(DEFAULT_SPEED))
      ...
      try {
        ...
        if (options.resume) {
          ...
          if (isNaN(rate) || rate < MIN_RATE || rate > MAX_RATE) {
            console.error(`Error: Invalid rate '${options.rate}'. Must be between ${MIN_RATE} and ${MAX_RATE}`);
            process.exit(1);
          }
        ...
        if (options.chunked) {
          ...
          await speakSimple(text, options.voice as Voice, options.model, rate, options.output);
        }
      } catch (error: unknown) {
